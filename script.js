...
let state = { 
    ...
    playerMarker: null, currentHeading: 0, targetObj: null, 
    navLine: null, autoCenter: true, gameMode: "DOMINATION" 
};
let activeMarkers = [];
let map;

// 1. GESTIONE PERSISTENZA LOGIN
window.onload = () => {
    initMap();
    ...
    if (saved) {
        ...
        enableSensorsAndStart(); // Autologin
    }
};
...
function handleRotation(e) {
    ...
    if (compass) {
        ...
        // Ruota solo il contenitore interno, non interferisce con il drag di Leaflet
    ...
    setTimeout(() => {
        ...
        navigator.geolocation.watchPosition(p => {
            ...
            if(!state.playerMarker) {
                ...
                if (state.autoCenter) map.panTo(pos); // panTo preserva lo zoom
                if (state.targetObj) updateNavigationDisplay();
            }
...
function updateUI(r) {
    ...
    (r.objectives || []).forEach(obj => {
        ...
        li.innerHTML = `${obj.name} <span>${obj.owner}</span>`;
...
// Funzioni Master & Utils (Bussola, Navigazione etc rimangono invariate come logica)
...
async function loadConfigFromServer() { /* ... codice caricamento obiettivi già esistente ... */ }
function reloadMap() { map.invalidateSize(); }
function stopNavigation() { state.targetObj = null; if(state.navLine) map.removeLayer(state.navLine); document.getElementById("nav-panel").style.display = "none"; }
